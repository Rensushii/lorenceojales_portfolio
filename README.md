...
## 3. Navigation model: paginated sections, not free scroll

The page no longer scrolls as one long document. `#pageOuter` (the `<main>`
element) fills the viewport below the fixed header, and every top-level
section is stacked on top of the others at `inset: 0`. Only the section with
`.active` is visible — switching sections happens **only** by clicking a nav
link (top bar, mobile menu, the logo, or the hero's "View Projects" button),
handled by `src/js/pagination.js`.

If a section's content is taller than the viewport (the certifications grid,
for example), that section scrolls internally on its own — the scroll never
spills into the next section, since each one is its own isolated
`overflow-y: auto` box (`overscroll-behavior: contain` in `pagination.css`
stops any bounce-through too).

`src/js/reveal.js` was changed to match: entrance animations now replay each
time a section is activated (rather than firing once via
`IntersectionObserver` on scroll), so switching back to a section you've
already seen re-triggers its fade-ins/stagger.

To go back to normal continuous scrolling instead, delete the
`@import './pagination.css';` line from `src/styles/main.css`, drop the
`id="pageOuter"` from the `<main>` tag in `src/main.js`, and swap
`initPagination()` back out for a scroll-based nav handler (the previous
version of `nav.js`, before this change, did exactly that).

## 4. What still works exactly as before
...
## 5. UI/UX changes made in the redesign
...
## 6. Data you may want to move further
